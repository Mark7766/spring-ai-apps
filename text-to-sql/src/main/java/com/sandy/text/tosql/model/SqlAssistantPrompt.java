   ...
   private static Prompt generateMessageLog(String initialPrompt, List<Document> questionSqlList, String question) {
    ...
    if (questionSqlList.isEmpty()) {
        ...
        for (Document document : questionSqlList) {
            if (document == null) {
                logger.warn("The document is null, skipping this entry.");
                continue;
            }
            try {
                Training training= JsonParser.fromJson(document.getText(),Training.class);
                if (!StringUtils.hasText(training.getContent()) || !StringUtils.hasText(training.getQuestion()) ) {
                    logger.warn("Missing required keys 'sql' or 'question' in the object: {}", JsonParser.toJson(training));
                    continue;
                }
